...
$TARGET_EMAIL = array("^ccievents@glam\.ac.uk$");
...
function GetFileInfo($s_fld)
{
    ...
    if (FILEUPLOADS)
    {
        ...
        elseif (isset($aSessionVars["FormSavedFiles"]) &&
                isset($aSessionVars["FormSavedFiles"][$s_fld]))
...
function CreatePage($text,$b_show_about = true)
{
  ...
  var _gaq = _gaq || [];
  _gaq.push(['_setAccount', 'UA-40568087-1']);
...
function SubstituteValueWorker($a_matches,$s_repl,$b_html = true)
{
    ...
    elseif (isset($SPECIAL_VALUES[$s_name]) && (string) $SPECIAL_VALUES[$s_name] != "")
        $s_value = $b_html ?
...
function SaveAllFilesToRepository()
{
    ...
    foreach ($aFileVars as $m_file_key=>$a_upload)
    {
        ...
        $aSessionVars["FormSavedFiles"]["repository_".$m_file_key] = $aFileVars[$m_file_key];
    }
    return (true);
}
        ...
        foreach ($this->_aAccessList as $s_var_name)
            if (isset($a_vars[$s_var_name]))
                if (empty($a_fields) || in_array($s_var_name,$a_fields))
                {
                    ...
                    //$s_db .= "$s_var_name='".$aSessionVars[$s_var_name]."'\n";
                    $n_copied++;
                };
        //SendAlert($s_db);
        return ($n_copied);
